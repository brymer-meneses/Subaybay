  ...
  <div class="m-2 rounded-lg" style="background-color:{mainBGColor};">
    ...
    >
      ...
      {:else}
        <!-- svelte-ignore a11y-no-static-element-interactions svelte-ignore a11y-click-events-have-key-events -->
        <div
          class="p-2 rounded-tr-lg flex-grow cursor-pointer"
    ...
    {#if stageNumber > 0}
      <div class="m-0 p-0 rounded-bl-lg rounded-br-lg" style="flex-grow;">
          ...
          {#each substages as substageData, index}
            {#if index > 0}
              <ConfigStageButton
                ...
                onButtonClick={() => deleteSubstage(index)}
                onEditHandler={editHandler}
                {users}
              />
            {/if}
      ...
      {#if buttonType != ButtonType.Disabled}
        <ConfigStageButton
          ...
          stageIndex={substages.length - 1}
          stageName={"Add New"}
          handlerIndex={0}
          isLast={true}
          isRenamable={false}
          onButtonClick={addSubstage}
          {users}
        />
      {/if}
    {/if}
