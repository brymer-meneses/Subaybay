...
::: warning
The following sections can be a little bit technical since it uses some programming and markup languages, but these will be explained as simple as possible.
:::

The following steps show how the hardcoded email template can be modified within the codebase. Note that afterwards, for the changes to be seen within the app, the system is required to undergo [redeployment](/maintenance/redeployment).

The hardcoded email template is written as a svelte component, named as `ConfirmationEmail.svelte` and is located at
...
1. First, open the codebase (the whole project folder) in any IDE or text editor, e.g. VS Code or Notepad++.
2. Find the `ConfirmationEmail.svelte` component. As of writing, this should be located at `/app/src/lib/components/email/ConfirmationEmail.svelte`.
3. Once you opened the component, you should see two sections in which the svelte code is divided into. These are the **script section** (within `<script lang="ts">...</script>`) and the **Html section** (within `<Html lang="en"><Text>...</Text></Html>`).
   The script section is where the data for a specific email is imported while, the Html section contains the main structure of the email template.

   ![sections](sections.png)
   ::: tip
   The Html Section mostly uses basic HTML elements and the script section uses basic Typescript syntax.
   :::

4. Modify each part to how the template should be by following the guides in the [Script section](#script-section-guide) and [Html section](#html-section-guide).

## Script Section Guide

The script section uses basic Typescript syntax. However, programming concepts are not needed at minimum as we are only using those to import _props_ (at least for the default email template). Think of these props as *prop*erties passed on to this component. Props are the data that we will be using. In the future, if the template needs further data processing, the script section can be added additional lines of code to handle it (with Typescript).
...
| `<img src={...}/>`               | This element is used for adding images. The source `src` takes what image will be displayed.                                         | Please see the code block below.                                                            |
